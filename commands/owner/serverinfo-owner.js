...
const { SlashCommandBuilder, MessageFlags, ContainerBuilder, TextDisplayBuilder, SectionBuilder, ThumbnailBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, SeparatorBuilder, SeparatorSpacingSize } = require('discord.js');
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('serverinfo-owner')
        .setDescription('<:Lock:1473038513749491773> Owner Only: Get detailed info about any server')
    ...
    async execute(interaction) {
        ...
        if (!guild) {
            return interaction.reply({ content: `<:Cancel:1473037949187657818> Bot is not in server with ID: \`${guildId}\``, flags: MessageFlags.Ephemeral });
        }
