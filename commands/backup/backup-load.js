...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (result.success) {
                return interaction.editReply({ components: [new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`# <:Checkedbox:1473038547165384804> Backup Restored\n\n**<:Box:1473039115581915256>** \`${result.backupName}\`\n**<:Folderopen:1473039552783323348>** ${result.restoredCount} configs restored`))], flags: MessageFlags.IsComponentsV2 });
            }
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId.split(':')[1] === 'confirm') {
                try {
                    ...
                    if (result.success) {
                        return i.update({ components: [new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`# <:Checkedbox:1473038547165384804> Backup Restored\n\n**<:Box:1473039115581915256>** \`${result.backupName}\`\n**<:Folderopen:1473039552783323348>** ${result.restoredCount} configs updated\n\n> All systems have been refreshed with backup data.`))], flags: MessageFlags.IsComponentsV2 });
                    }
