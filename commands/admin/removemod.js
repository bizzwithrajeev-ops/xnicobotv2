const { ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, SeparatorSpacingSize, MessageFlags } = require('discord.js');
const { COLORS, buildErrorResponse } = require('../../utils/responseBuilder');
const trust = require('../../utils/trustManager');

module.exports = {
    prefix: 'removemod',
    description: 'Remove a user or role from the Moderator trust list',
    usage: 'removemod <@user|@role|userId>',
    category: 'admin',
    aliases: ['remove-mod', 'delmod'],

    async executePrefix(message, args) {
        if (!trust.isServerOwner(message.guild, message.author.id)) {
            const container = buildErrorResponse('Permission Denied', 'Only the **server owner** or **second owner** can use this command.');
            return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        }

        try {
            let targetUser = message.mentions.users.first() || null;
            let targetRole = message.mentions.roles.first() || null;

            if (!targetUser && !targetRole && args[0]) {
                const id = args[0].replace(/[<@!&>]/g, '');
                try { targetUser = await message.client.users.fetch(id); } catch {
                    try { targetRole = await message.guild.roles.fetch(id); } catch {
                        const container = buildErrorResponse('Not Found', 'Could not find that user or role. Provide a valid mention or ID.');
                        return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
                    }
                }
            }

            if (!targetUser && !targetRole) {
                const container = new ContainerBuilder()
                    .setAccentColor(COLORS.INFO)
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent(
                        `# <:Shield:1473038669831995494> Remove Moderator\n\n` +
                        `<:Infocircle:1473038519029989588> **Usage:** \`removemod @user\` or \`removemod @role\`\n\n` +
                        `<:Caretright:1473038207221502106> Removes a user or role from the trusted moderator list.\n` +
                        `<:Caretright:1473038207221502106> The **Trusted Moderator** role will be revoked.\n` +
                        `<:Caretright:1473038207221502106> All moderator-level bot access will be removed.`
                    ))
;
                return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }

            const targetId = targetUser ? targetUser.id : targetRole.id;
            const targetType = targetUser ? 'user' : 'role';
            const targetName = targetUser ? targetUser.username : targetRole.name;

            if (!trust.getList(message.guild.id, 'mods').some(e => e.id === targetId)) {
                const container = buildErrorResponse('Not a Moderator', `**${targetName}** is not in the moderator list.`);
                return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }

            const confirmContainer = new ContainerBuilder()
                .addTextDisplayComponents(new TextDisplayBuilder().setContent(
                    `# <:Infotriangle:1473038460456800459> Confirm: Remove Moderator\n\n` +
                    `<:Caretright:1473038207221502106> **Target:** ${targetUser || targetRole} (\`${targetId}\`)\n` +
                    `<:Caretright:1473038207221502106> **Type:** ${targetType === 'user' ? '<:User:1473038971398520977> User' : '<:Userplus:1473038912212435086> Role'}\n` +
                    `<:Caretright:1473038207221502106> **Requested by:** ${message.author.username}\n\n` +
                    `### What will happen:\n` +
                    (targetType === 'user'
                        ? `- <:Caretright:1473038207221502106> User will be removed from the **Moderator trust list**\n- <:Caretright:1473038207221502106> The **Trusted Moderator** role will be revoked\n- <:Caretright:1473038207221502106> All moderator-level bot access will be removed`
                        : `- <:Caretright:1473038207221502106> Role will be unmarked as a **Trusted Moderator** role\n- <:Caretright:1473038207221502106> Members with only this role lose moderator-level bot access`) +
                    `\n\n-# Are you sure you want to proceed?`
                ));

            await trust.withConfirmation(message, confirmContainer, async (i) => {
                const result = trust.removeFromList(message.guild.id, 'mods', targetId);
                if (!result.success) {
                    const fail = new ContainerBuilder().setAccentColor(COLORS.ERROR)
                        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# <:Cancel:1473037949187657818> Failed\n\n${result.message}`));
                    return i.update({ components: [fail], flags: MessageFlags.IsComponentsV2 });
                }

                let roleNote = '';
                if (targetType === 'user') {
                    const rr = await trust.removeTrustRole(message.guild, targetId, 'mods');
                    roleNote = rr.success ? `\n<:Caretright:1473038207221502106> **Role Revoked:** Trusted Moderator` : `\n<:Infotriangle:1473038460456800459> Could not revoke role: ${rr.error}`;
                }

                if (targetUser) {
                    await trust.notifyUser(message.client, targetUser.id,
                        `<:Notificationon:1473038417691676784> You have been **removed** from the **Moderator trust list** in **${message.guild.name}** by **${message.author.username}**.\n> Your moderator-level access has been revoked.`
                    );
                }

                const success = new ContainerBuilder()
                    .setAccentColor(COLORS.SUCCESS)
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent(
                        `# <:Checkedbox:1473038547165384804> Moderator Removed Successfully\n\n` +
                        `<:Caretright:1473038207221502106> **${targetType === 'user' ? '<:User:1473038971398520977> User' : '<:Userplus:1473038912212435086> Role'}:** ${targetUser || targetRole} (\`${targetId}\`)${roleNote}\n` +
                        `<:Caretright:1473038207221502106> **Removed by:** ${message.author.username}\n\n` +
                        `<:Caretright:1473038207221502106> They no longer have moderator-level trust in this server.`
                    ))
;
                await i.update({ components: [success], flags: MessageFlags.IsComponentsV2 });
            });
        } catch (error) {
            console.error('[RemoveMod] Error:', error);
            const container = buildErrorResponse('Error', 'An error occurred while executing this command.', error.message);
            return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        }
    }
};
